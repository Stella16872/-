...
| 点 Photos | **相册** | 加照片、按月分组、点开看大图并写一句说明；照片存本机 IndexedDB |
...
**换头像**：设置里「头像」那两行点「换照片」，选一张就好。
图片会自动居中裁成方形、缩到 220px 再存，只占几 KB。
没设置时是「金粉渐变圆 + 名字首字」的占位。

**照片存哪**：相册里的照片放在浏览器的 IndexedDB（localStorage 装不下），
导入时自动缩到长边 1600、转 JPEG，一张四五 MB 的手机原图大约压到 20 KB。
说明文字和日期跟其他数据一样在 localStorage。
注意：导出备份**不含照片本身**，只含说明和日期。
